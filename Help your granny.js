...
function tour(friends, fTowns, distTable) {
  ...
  friends.forEach((friend) => {
    const townIndex = fTowns.findIndex((town) => town[0] === friend)
    if (townIndex !== -1) townsSorted.push(fTowns[townIndex])
  })

  const distsSorted = []

  townsSorted.forEach((town) => {
    const distIndex = distTable.findIndex((dist) => dist === town[1])
    distsSorted.push(distTable[distIndex + 1])
  })

  let distance = distsSorted[0]

  for (let i = 0; i < distsSorted.length - 1; i++) {
    distance += Math.sqrt(
      Math.pow(distsSorted[i + 1], 2) - Math.pow(distsSorted[i], 2)
    )
  }

  distance += distsSorted[distsSorted.length - 1]
